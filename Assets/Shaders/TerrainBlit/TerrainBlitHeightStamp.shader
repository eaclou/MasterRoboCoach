	...
	SubShader
	{
		...
		Pass
		{
			...
			{	
				...
				//float4 cc = tex2D(_MainTex, i.uv + float2(0, 0)); // Centre Centre  // Original Height
				...
				//float radius = 100;
				//float distanceMask = smoothstep(_RadiusStartFade, _RadiusEndFade, length(coords - _WorldPivot.xz) + Value2D(coords, 34).yz * 0.01);
				//distanceMask;
				//cc.x = 0;
				//cc.y = 0;
				//cc.z = 0;
				...
				//float4 newHeightSample = tex2D(_MainTex, i.uv + float2(0, 0)); 
				//float4 newHeights = lerp(cc, float4(0,0,0,0), saturate(distanceMask));
				...
				newHeight = GetNoiseSample(newTexSampleParamsCBuffer, lerp(coords - _WorldPivot.xz, coords - _WorldPivot.xz, _NewTexFlowAmount));				
				//float mask1Value = 1;
				...
				float newRockHeight = newHeight.x + _AltitudeOffset; // * saturate(mask1Value.x) * saturate(mask2Value.x);

				newRockHeight = lerp(float4(0,0,0,0), newRockHeight, 1.0 - distanceMask);
				...
				// Average:
					finalHeight = lerp(finalHeight, newRockHeight, 1.0 - distanceMask);
