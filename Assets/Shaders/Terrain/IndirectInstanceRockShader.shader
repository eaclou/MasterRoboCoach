Shader "Instanced/IndirectInstanceRockShader" {
    ...
    SubShader {
        ...
        StructuredBuffer<TransformData> instancedRocksCBuffer;
		StructuredBuffer<float4x4> instancedRocksInvMatrixCBuffer;
    #endif
        ...
        void setup()
        {
            ...
            float4x4 rotationMatrix = float4x4(
                cos(data.rotation.y)*cos(data.rotation.z), -cos(data.rotation.y)*sin(data.rotation.z), sin(data.rotation.y), 0,
                cos(data.rotation.x)*sin(data.rotation.z)+sin(data.rotation.x)*sin(data.rotation.y)*cos(data.rotation.z), cos(data.rotation.x)*cos(data.rotation.z)-sin(data.rotation.x)*sin(data.rotation.y)*sin(data.rotation.z), -sin(data.rotation.x)*cos(data.rotation.y), 0,
                sin(data.rotation.x)*sin(data.rotation.z)-cos(data.rotation.x)*sin(data.rotation.y)*cos(data.rotation.z), sin(data.rotation.x)*cos(data.rotation.z)+cos(data.rotation.x)*sin(data.rotation.y)*sin(data.rotation.z), cos(data.rotation.x)*cos(data.rotation.y), 0,
                0, 0, 0, 1);
            ...
            unity_WorldToObject = instancedRocksInvMatrixCBuffer[unity_InstanceID];
        #endif
        }
        ...
        void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			col = instancedRocksCBuffer[unity_InstanceID].worldPos;	
